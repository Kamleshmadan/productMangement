...
- **First-time setup:** you need at least one user row in the database (or register via `/auth/register` if your security rules allow the caller to hit that endpoint). Use a role value that maps to Spring Security roles (e.g. `ADMIN` or `USER`).
